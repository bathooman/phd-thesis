...
While symbolic execution provides a systematic foundation for testing protocol conformance, applying it effectively to real-world network protocol implementations presents several challenges.
...
Classical symbolic execution is designed for stateless programs, in which each input is processed independently of all previous inputs.
In protocols, however, validating certain requirements requires observing specific sequences of message exchanges rather than a single exchange.
Applying symbolic execution to network protocol implementations therefore requires reasoning about how earlier symbolic inputs influence subsequent behavior.
...
Classical symbolic execution, however, cannot directly reason about these interactions, since they take place outside the control of the program under analysis.
...
Network protocol specifications, however, are written in natural language, and ambiguities, cross-references, and conditional requirements make it difficult to translate these descriptions into formal properties that can be checked automatically.
As a result, defining precise conformance criteria that capture the intent of the specification remains a major challenge for applying symbolic execution to protocol testing.
...
\Cref{sec:background} provides background on software testing and symbolic execution, describing both traditional and modern approaches to protocol testing.
Chapters~\ref{sec:paper1}~to~\ref{sec:paper4} summarize the research papers included in this thesis, each addressing a specific aspect of applying symbolic execution to protocol conformance testing.
\Cref{sec:related} reviews related work.
\Cref{sec:conclusion} presents the overall conclusions drawn from this research, followed by \Cref{sec:future}, which outlines possible directions for future work.
Finally, \Cref{sec:contributions} summarizes the main contributions of the thesis.