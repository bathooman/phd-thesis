...
Even when a deviation does not result in a direct vulnerability, it may still produce unpredictable or non-standard behavior when interacting with other implementations.
...
When requirements from the specification are expressed as assertions in the code or checked externally through monitors, symbolic execution can automatically detect deviations from expected behavior.
...
As a result, defining precise conformance criteria that capture the intent of the specification remains a major obstacle to applying symbolic execution effectively in protocol testing.
