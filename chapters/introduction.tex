...
A network protocol defines a set of rules that govern how distributed software and hardware systems communicate.
...
For example, a length field defines the number of bytes that follow, a flag bit can change the message type, and a sequence number links the message to previous communication in the interaction.
...
Despite these variations, each implementation must conform to the same rules of interaction for this particular protocol to ensure interoperability.
...
A protocol specification describes the syntax and semantics of messages, the constraints on field values, the permitted ordering of messages, and the state transitions that occur during communication. 
...
For example, RFC 6347~\cite[p. 9]{RFC6347} mandates that ``implementations \emph{MUST} either abandon an association or rehandshake prior to allowing the sequence number to wrap`` illustrating how specifications define concrete behaviors to preserve reliability and correctness. 
While a specification captures the abstract rules of communication, it is written in natural language and often includes explanatory text, loosely defined conditions, ambiguous phrasing, or even contradictory statements.
...
Unlike functional testing, which focuses on whether a component produces the correct output for a given input, conformance testing evaluates compliance with externally defined standards.
...
Despite providing a systematic foundation for testing protocol conformance, SE faces several challenges when applied to real-world network protocol implementations.
...
\Cref{chapter:background} provides background on software testing and SE, describing both traditional and modern approaches to protocol testing.
Chapters~\ref{chapter:paper1}~to~\ref{chapter:paper4} summarize the research papers included in this thesis, each addressing a specific aspect of applying SE to protocol conformance testing.
\Cref{chapter:related} reviews related work,
\Cref{chapter:conclusion} presents the overall conclusions drawn from this research, \Cref{chapter:future} outlines potential directions for future work, and \Cref{chapter:contributions} summarizes the main contributions of the thesis.