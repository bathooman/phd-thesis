...
If a concrete input causes $\mathtt{I}$ and $\mathtt{I'}$ to produce different responses, then there exist a Path-Output pair $\langle pc, o \rangle \in S_I$ and a Path-Output pair $\langle pc', o' \rangle \in S_{I'}$ such that both path conditions are satisfied by the same input, while the resulting responses differ.
