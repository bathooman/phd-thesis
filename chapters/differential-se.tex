...
Although a separate harness instance is constructed for each SUT, these harnesses share the same structure and differ only in the concrete SUT that is being tested.
...
These outputs include packets sent by the SUT or the absence of a response following the packet in which the field was made symbolic.
...
This design choice isolates the effect of a single symbolic input at a well-defined interaction point, which simplifies both symbolic exploration and the interpretation of discrepancies.
...
Given two SUTs, denoted $\mathtt{I}$ and $\mathtt{I'}$, the goal of differential SE is to identify common inputs that cause the two SUTs to produce different responses when a symbolic field is introduced at the same interaction point.
