...
To compare the behavior of different SUTs, it is necessary to ensure that symbolic field is introduced at the same point in the protocol interaction.
...
While it is possible to make multiple fields symbolic within a single campaign, doing so significantly increases the number of explored paths and makes it harder to attribute observed discrepancies to specific input values.
...
One of the witnesses indicates that, at the initial interaction point, upon receiving a first \emph{ClientHello} packet with a \rfc{sequence\_number} slightly larger than $2^{44}$, Mbed TLS responds with a \emph{HelloVerifyRequest}, indicating acceptance of the packet, while wolfSSL produces no response and appears to silently discard it.
According to the DTLS specification~\cite[p. 8]{RFC6347}, record sequence numbers are 48-bit values and are valid as long as they remain strictly less than $2^{48}$.
The sequence number produced by the witness therefore falls within the valid range.
...
For DTLS, the output-capture function inspects the parsed DTLS record produced by the SUT and records selected fields such as message types or alert codes, while excluding fields whose values are nondeterministic or configuration-dependent.
...
This assignment, together with the differing responses, yields a discrepancy witness as defined earlier in this chapter.
