...
In addition, not all protocol behaviors are defined by strict, explicitly stated rules.
...
The technique symbolically executes multiple implementations on shared symbolic packet inputs and observes their externally visible behavior.
When SE produces execution paths whose responses differ across implementations for a common input, the subsequent analysis reports a behavioral discrepancy and extracts concrete packet values that induce it.
...
The implementation of the differential SE technique builds directly on the monitor-based infrastructure introduced in \Cref{chapter:paper2}.
...
Monitors are no longer used to check specification requirements, but instead to observe packet exchanges and record execution outcomes that are later analyzed to identify behavioral discrepancies across implementations.
This reuse allows differential SE to be integrated into the existing framework while addressing aspects of protocol behaviors that are difficult to capture using specification-based oracles alone.
\section{Technique}
\subsection{Creating a Test Harness}\label{sec:paper3:test-harness}
...
For a given protocol role of the SUT, the same facilitator implementation is used for all tested SUTs.
Although a separate harness is constructed for each SUT, these harnesses share the same structure and differ only in the concrete SUT that is being tested.
Reusing the same facilitator ensures that all SUTs are subjected to identical inputs and interaction patterns, and that any observed behavioral differences are due to differences in the SUT implementations rather than from variations in their peers.
...
Assuming that multiple protocol implementations are available, basic differential testing can already be performed. 
Concretely, the SUT can be executed on a predefined interaction using the corresponding test harness, selected response packets can be observed, and pairwise differences can be flagged.
...
These outputs include packets sent by the SUT or the absence of a response after processing a packet whose field was made symbolic.
...
For example, in DTLS, one campaign may focus on the handshake for a specific cipher suite and investigate how the implementation responds to different values of the record sequence number in input, while another campaign may target a different cipher suite.
...
In differential SE, monitors do not encode protocol requirements and do not check assertions.
Instead, they are used to observe packet exchanges and to record selected responses of each implementation during SE.
In particular, monitors record packets generated by the SUT or the absence of a response and associate these responses with the symbolic inputs that led to them.
...
\begin{figure}[hb]
	\centering
		\begin{tikzpicture}[node distance=5.3cm]
			...
			(c1) edge node[above,  font=\footnotesize]{$
				\frac{
...
In the running example, we apply this technique to find differences between the Mbed TLS and wolfSSL servers.
To make the role of monitors in the differential setting concrete, \Cref{fig:monitor:dtls:sequence} shows the monitor used in the running example to observe DTLS server responses for different values of the \rfc{sequence\_number} field.
...
When such a packet is observed, the monitor records the server's response using the \texttt{record\_response} function and transitions to the terminal state \ST{End}.
...
To compare the behavior of different SUTs, it is necessary to ensure that a symbolic field is introduced at the same point in the protocol interaction.
To this end, we associate each symbolic field with a specific position in the packet sequence exchanged during the interaction.
This position, which we refer to as the \emph{index}, identifies the point during the interaction at which a packet field is made symbolic.
...
This mechanism is the same as the one used in the monitor-based technique described in \Cref{sec:paper2:protocol-specific}.
Both the selected index and the symbolic field become part of the path condition recorded by SE.
For every explored path, the SE engine records this path condition together with the response observed for the packet in which the field was made symbolic.

Finally, to limit the number of explored paths and to reduce the manual effort required for analysis, each differential SE campaign employs a single monitor that makes one instance of a protocol field symbolic.
All other instances of the same field in the interaction remain fixed to their concrete values.
This design choice isolates the effect of a single symbolic input at a well-defined interaction point, which simplifies both SE and the interpretation of discrepancies.

While it is possible to make multiple fields symbolic within a single campaign, doing so significantly increases the number of explored paths and makes it harder to attribute observed discrepancies to specific symbolic fields.
We therefore restrict each campaign to a single symbolic field instance in order to keep the analysis manageable and the resulting discrepancies easier to interpret.

\subsection{Finding and Analyzing Discrepancies}\label{sec:meth-discrepancies}
Given two SUTs, the goal of differential SE is to identify common inputs that cause the SUTs to produce different responses when a symbolic field is introduced at the same interaction point.
...
For every explored path, the SE engine records the path condition together with the response observed for the packet in which the field was made symbolic.
...
Both $\mathtt{pc}$ and $\mathtt{o}$ are parameterized by the symbolic field and the packet index.
...
Given the sets $S_I$ and $S_{I'}$ for two implementations $\mathtt{I}$ and $\mathtt{I'}$, we compare these sets to determine whether there exists a common input under which the two implementations produce different responses.
Because each Path-Output pair is associated with a specific packet index, comparisons are restricted to pairs that correspond to the same interaction point, ensuring that symbolic fields are introduced at the same point in the interaction.

If a concrete input causes $\mathtt{I}$ and $\mathtt{I'}$ to produce different responses, then there exist a Path-Output pair $\langle pc, o \rangle \in S_I$ and a Path-Output pair $\langle pc', o' \rangle \in S_{I'}$ such that both path conditions are satisfied by the same input, while the resulting responses to this specific input differ.
When such a concrete input instantiates the symbolic field, the formula
\begin{gather}
	\mathtt{(pc \, \land pc') \, \land \, (o \, \neq \, o')}\label{eq:discrepancy}
\end{gather}
is satisfied.

The conjunct $\mathtt{(pc \land pc')}$ ensures that such a common assignment to the symbolic input fields satisfies both path conditions, while the conjunct $\mathtt{(o \neq o')}$ ensures that this assignment leads to different responses.
For each pair of Path-Output pairs $\langle pc, o \rangle$ and $\langle pc', o' \rangle$ with the same packet index, we check the satisfiability of \fref{eq:discrepancy} using an SMT solver.
...
Our technique produces a witness for every pair of Path-Output pairs that satisfies \fref{eq:discrepancy}.

\paragraph{Analyzing Discrepancies for Bugs.}
For each discrepancy witness, the protocol specification is consulted to interpret the observed behaviors.
At this stage, the RFC serves as a reference for interpretation rather than as an executable oracle.
Several outcomes are possible: both implementations may be compliant with the specification, both may be non-compliant, or the discrepancy may arise from underspecified or ambiguous parts of the protocol where multiple behaviors are permitted.

If the analysis indicates that one implementation is non-compliant while the other behaves in accordance with the specification, we validate the discrepancy by executing the test harness on the unmodified implementation.
...
For Mbed TLS, 35 Path-Output pairs are generated, while for wolfSSL, 28 Path-Output pairs are generated.
%Although these numbers may appear large, all steps up to discrepancy interpretation can be automated.
Instantiating \fref{eq:discrepancy} for these Path-Output pairs yields two satisfying assignments, each forming a discrepancy witness.
...
One of the witnesses indicates that, upon receiving the first \emph{ClientHello} packet with a \rfc{sequence\_number} slightly larger than $2^{47}$, Mbed TLS responds with a \emph{HelloVerifyRequest}, indicating acceptance of the packet, while wolfSSL produces no response and appears to silently discard it.
According to the DTLS specification~\cite[p. 8]{RFC6347}, record sequence numbers are 48-bit values and must remain strictly less than $2^{48}$; the sequence number produced by the witness satisfies this numeric constraint.
Consulting the specification reveals that Mbed TLS handles this packet correctly, whereas wolfSSL incorrectly rejects a packet whose sequence number is large but still valid.
...
In particular, the same packet interception mechanism, monitor invocation model, and protocol-specific parsing infrastructure are used.
...
For each SUT, SE is performed within a dedicated test harness consisting of the SUT and a facilitator, as described in \Cref{sec:paper3:test-harness}.
...
As in the monitor-based technique presented in \Cref{chapter:paper2}, the harness mediates all communication between the facilitator and the SUT; the concrete interception and packet-handling mechanisms are described in \Cref{sec:paper3-common-infra}.
...
Although a separate harness is required for each library, a harness can typically be reused across multiple versions of the same implementation with little or no modification.
...
The loop also terminates if a fatal alert is generated or if an iteration produces no new packet.
...
\subsubsection{Common Infrastructure}\label{sec:paper3-common-infra}
As in the monitor-based technique (\Cref{sec:paper2:generic}), packet exchange between the facilitator and the SUT is mediated by a proxy that intercepts POSIX socket calls.
...
Beyond the common infrastructure, our technique relies on several protocol-specific components.
...
Similar to the monitor-based framework, nondeterministic selection of the packet index is implemented by the infrastructure rather than explicitly encoded in the monitor.
...
Conceptually, this function operates on the structured representation of a protocol message produced by the SUT, extracts a selected set of response fields, and records their values as the observable output for the current execution path.
The choice of which fields to record is protocol-specific and follows the guidelines described in \Cref{sec:outputfieldselection}.

For DTLS, the output-capture function inspects the parsed DTLS record produced by the SUT and captures selected fields such as message types or alert codes, while excluding fields whose values are nondeterministic or depend on configuration.
Analogous output-capture functions are implemented for other protocols by selecting and recording the corresponding protocol-specific response fields.
...
Each pair consists of an SMT formula encoding the path condition and a symbolic representation of the selected response fields recorded by the output-capture function.
...
For every pair of SUTs, the script iterates over pairs of paths that correspond to the same packet index and checks the satisfiability of the discrepancy condition represented by \fref{eq:discrepancy} using the SMT solver Z3.
...
This assignment, together with the differing responses, yields a discrepancy witness.
...
In our running example, this strategy reduces the number of reported discrepancies from ten to two.

\subsection{Optional SUT Adaptations}
...
As in the monitor-based technique described in \Cref{chapter:paper2}, we apply a small set of optional adaptations to the SUT to improve performance without affecting the analysis.
...
In our evaluation, differential SE uncovered nonconformance issues that were not detected by the monitor-based approach.
Several of these nonconformances arose in parts of the protocol that are underspecified or loosely defined in the RFC, making them difficult to encode accurately as specification-based monitors.
...
The final chapter examines these techniques side by side by applying them to the same set of protocol requirements and implementations.
Using the Constrained Application Protocol (CoAP) as a case study, it analyzes their effectiveness in identifying requirement violations in resource-constrained environments.
