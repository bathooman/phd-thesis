...
Ensuring that protocol implementations conform to their specifications has long been an important goal, motivated by the fact that even subtle deviations can lead to interoperability issues or security vulnerabilities.
Traditional protocol conformance testing approaches are typically specification driven and rely on abstract models or explicitly encoded properties derived from specification documents such as RFCs.
...
While MBT can provide strong guarantees when accurate models are available, constructing and maintaining such models for complex evolving protocols is labor intensive.
...
To address this, several extensions to greybox fuzzing have been proposed, including AFLNET~\cite{AFLNET@ICST-20}, StateAFL~\cite{StateAFL@EmpirSoftEng-22}, SGFuzz~\cite{SGFuzz@USENIX-22}, and SnapFuzz~\cite{SnapFuzz@ISSTA-22}.
...
Despite these advances, the primary objective of most fuzzing-based techniques remains the detection of runtime errors such as crashes, assertion failures, or hangs.
They generally do not aim to check semantic protocol requirements derived from specifications nor do they provide guarantees about the absence of requirement violations.
...
This approach had been successfully applied to a variety of protocols, including DTLS~\cite{DTLS-Fuzzer@ICST-22}, TCP~\cite{FH2017,FJV2016}, SSH~\cite{SSH@SPIN-2017}, OpenVPN~\cite{OpenVPN@EuroSPW-2018}, IPSec~\cite{IPSec@IEEEAccess-2019}, QUIC~\cite{Prognosis@SIGCOMM-21,rasool2019}, and EDHOC~\cite{EDHOC-Fuzzer@ISSTA-23}.
...
Symbolic execution (SE) was introduced in the 1970's as a program analysis technique that explores program behavior by treating selected inputs as symbolic rather than concrete values~\cite{King@CACM-76}.
...
Several approaches address this limitation by guiding SE more explicitly toward requirement-relevant behaviors.
