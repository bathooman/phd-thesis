...
For fuzzing, the harness supplies mutated variants of the captured packets, while for SE it designates selected fields as symbolic and relies on the symbolic executor to explore feasible execution paths.
...
Once a set of requirements has been extracted from these specifications, they can be translated into executable assertions that are evaluated during a protocol interaction.
...
\begin{equation*}
	\begin{split}
		...
		& \scriptstyle \mathtt{else \, if} (\neg \, \rfcm{first\_response} \, \land \, \rfcm{p_{in}.code} \, \in \, \rfcm{REQ\_CODES})\\[-5pt]
...
\end{equation*}
%
The assertion is evaluated only for subsequent relevant requests, after the server’s preferred block size has been established by the first relevant response. In the formulation presented in Paper~\URoman{4}, these scoping conditions are implicit, whereas they are made explicit here for clarity.
...
In our experiments, this behavior did not prevent the fuzzer from generating valid test cases that successfully exercised multi-packet interactions and triggered assertion violations.
When packet parsing succeeds, this design allows the fuzzer to mutate a single input buffer while still exercising a multi-packet protocol interaction.
