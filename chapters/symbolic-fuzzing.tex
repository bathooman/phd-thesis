\chapter{Two Paths to Conformance: Fuzzing and Symbolic Execution}\label{chapter:paper4}
...
At the same time, fuzzing lacks the ability to reason explicitly about protocol requirements or to systematically target specific semantic conditions.
...
In this chapter, protocol requirements are encoded as assertions over the packets exchanged during an interaction.
...
\end{equation*}
%
Here, \rfc{REQ\_CODES} denotes the set of CoAP request codes.
The function $\rfcm{BlockSize}$ extracts the block size indicated by the Block1 option of a packet, and \rfc{ERR\_CODES} denotes the set of CoAP error response codes.
...
To test protocol requirements using fuzzing, we employ coverage-guided fuzzing to generate inputs that may trigger violations of the encoded assertions.
...
This design allows the fuzzer to mutate a single input buffer while still exercising a multi-packet protocol interaction.
...
To enable symbolic manipulation of protocol fields, incoming packets are first parsed into structured data representations using a CoAP-specific parser.
Selected fields in these structures are then made symbolic using KLEE's API.
After symbolic values have been introduced, the modified data structures are serialized back into byte buffers, which are subsequently processed by the protocol implementation.
This parsing and serialization step allows SE to operate at the level of protocol fields rather than raw byte arrays.
...
Across the evaluated requirements, the two techniques identified a total of nine distinct nonconformances in the tested implementations.
...
A detailed description of the experimental setup, the evaluated implementations, and the complete set of discovered issues is provided in \textcolor{red}{Paper~4}, included later in this thesis.
